...
def compute_pass_at_k(n: int, c: int, k: int) -> float:
    """
    Compute pass@k metric using the unbiased estimator from the Codex paper.
    
    pass@k = 1 - C(n-c, k) / C(n, k)
    
    Args:
        n: Total number of samples
        c: Number of correct samples (where acc/reward >= threshold, typically 1.0)
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                # Skip string values (check first non-None value)
                if isinstance(var_vals_clean[0], str):
                    continue
                ...
                # Compute pass@k for variables that represent accuracy (binary 0/1 or scores >= 1.0)
                # Count correct samples: values >= 1.0 are considered "correct"
                if var_name in ("acc", "reward"):
                    n_correct = sum(1 for v in var_vals_clean if v >= 1.0)
                    # Compute pass@k for k = 1, 2, 4, ..., n_resps
                    ks = [1]
                    k = 2
                    while k < n_resps:
                        ks.append(k)
                    ...
                    for k in ks:
                        if k <= n_resps:
                            metric[f"pass@{k}"] = compute_pass_at_k(n_resps, n_correct, k)
